...
export async function POST(request: Request) {
  ...
  try {
    ...
    const record = (rows as Array<{ id: string; name: string; email: string; role: "farmer" | "researcher" | "admin" | "user"; status: "active" | "suspended"; passwordHash: string }>)[0];
    ...
    const user = { id: record.id, email: record.email, name: record.name, role: assignedRole, status: record.status };
