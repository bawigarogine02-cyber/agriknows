...
export async function POST(request: Request) {
  let body: { name?: string; email?: string; password?: string; confirmPassword?: string; role?: string };
  try {
    body = (await request.json()) as { name?: string; email?: string; password?: string; confirmPassword?: string; role?: string };
  } catch {
  ...
  const assignedRole: "farmer" | "researcher" = body.role === "researcher" ? "researcher" : "farmer";
  const user = { id: randomUUID(), name: body.name.trim(), email: body.email.trim().toLowerCase(), role: assignedRole, status: "active" as const };
  try {
