...
export type SessionUser = { id: string; email: string; name: string; role: "admin" | "user"; status: "active" | "suspended" };
...
export function verifySessionValue(value: string | undefined): SessionUser | null {
  ...
  try {
    ...
    return { id: parsed.id, email: parsed.email, name: parsed.name, role: parsed.role === "admin" ? "admin" : "user", status: parsed.status === "suspended" ? "suspended" : "active" };
  } catch {
    return null;
  }
}
